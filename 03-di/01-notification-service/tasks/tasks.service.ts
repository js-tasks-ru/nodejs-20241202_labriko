...
export class TasksService {
  ...
  async createTask(createTaskDto: CreateTaskDto) {
    ...
    this.notificationsService.sendEmail(to.email, "Новая задача", `Вы назначены ответственным за задачу: "${title}"`)
  ...
  async updateTask(id: string, updateTaskDto: UpdateTaskDto) {
    ...
    this.notificationsService.sendSMS(to.phone, `Статус задачи "${task.title}" обновлён на "${task.status}"`)
